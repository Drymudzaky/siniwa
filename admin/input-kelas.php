...
<html lang="en">
  ...
  $query_kelas = mysql_query("SELECT * FROM kelas order by kode_ksk desc")or die(mysql_error()); 
  $row_kelas = mysql_fetch_array($query_kelas);
  $totalrow_kelas = mysql_num_rows($query_kelas);

if ($totalrow_kelas > 0) {
  $kodeksk_terakhir = substr($row_kelas['kode_ksk'], -3);
  $nourut = $kodeksk_terakhir+1;
  $isikodeksk ="K"."00".$nourut;
                  ...
                  <div class="form-panel">
                      ...
                      <form class="form-horizontal style-form" action="insert-kelas.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
                          <div class="form-group">
                              ...
                              <div class="col-sm-10">
                                  <input name="kode_ksk" type="text" id="kode_ksk" class="form-control" placeholder="Isi dengan ex : K001 dst." autofocus="on" required="required" value="<?php echo $isikodeksk;?> " readonly/>
                              </div>
                          </div>
                          <div class="form-group">
                          ...
                          <div class="form-group">
                              ...
                              <div class="col-sm-10">
                                   <input name="kelas" type="text" id="kelas" class="form-control" placeholder="Nama Kelas" required />
                              </div>
                          </div>
                          <div class="form-group">
                              ...
                              <div class="col-sm-10">
                                  <select name="level_ksk" id="level_ksk"  class="form-control" required />
                                    ...
                                    <?php
                                    $sql = mysql_query("SELECT * FROM guru ORDER BY level_ksk ASC");
                                    if(mysql_num_rows($sql) != 0){
                                    while($data = mysql_fetch_assoc($sql)){
                                    echo '<option value='.$data['level_ksk'].'>'.$data['level_ksk'].'</option>'; }
                                    $kode = $data['level_ksk'];
                                    }
                                    ?>
                                  </select>
